...
CREATE TABLE `daisy_user` (
  ...
  `i_concern_limit` int(11) NOT NULL DEFAULT '0' COMMENT '关注上限',
  `i_query_limit` int(11) NOT NULL DEFAULT '0' COMMENT '查询的企业个数限制',
  `s_alt_email` text COMMENT '变更接收邮件',
  PRIMARY KEY (`id`),
  UNIQUE KEY `i_unique_code_ctx` (`s_code`) USING BTREE,
  UNIQUE KEY `i_unique_mial_ctx` (`s_email`) USING BTREE
...
-- ----------------------------
-- DROP TABLE IF EXISTS `risk_api_invoke_log`;
-- CREATE TABLE `risk_api_invoke_log` (
--   `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
--   `s_visit_code` varchar(50) NOT NULL COMMENT '来访客户编码',
--   `s_provider_code` varchar(50) NOT NULL COMMENT '接口商编码',
--   `s_api_code` varchar(100) NOT NULL COMMENT '方法编码',
--   `i_is_success` tinyint(1) NOT NULL COMMENT '是否成功1:成功  0:失败',
--   `s_message` varchar(10000) DEFAULT NULL COMMENT '成功或失败信息内容',
--   `s_transaction_id` varchar(50) NOT NULL COMMENT '流水号',
--   `s_invode_time` char(19) NOT NULL DEFAULT '' COMMENT '访问时间',
--   `i_call_type` tinyint(4) NOT NULL DEFAULT '0' COMMENT '0:接口调用 1:调试调用',
--   `i_cost_mills` int(11) NOT NULL DEFAULT '100' COMMENT '接口调用花费时长:毫秒',
--   PRIMARY KEY (`id`),
--   KEY `i_log_user_api` (`s_visit_code`,`s_api_code`) USING BTREE
-- ) ENGINE=InnoDB AUTO_INCREMENT=8782 DEFAULT CHARSET=utf8;
...
CREATE TABLE `risk_enterprise_push` (
  ...
  `reqest_time` char(19) DEFAULT NULL,
